...
def save_uploaded_file_temporarily(uploaded_file):
    ...
    safe_name = getattr(uploaded_file, "name", "audio.wav").replace(" ", "_")
...
def save_feedback_example(source_audio_path, selected_label, selected_clips, result, user_note):
    ...
    with open(FEEDBACK_LOG_CSV, "a", newline="", encoding="utf-8") as csvfile:
        ...
        writer.writerow([
            datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "project_milo",
...
def extract_60dim_features(y, sr):
    ...
    return np.concatenate([
        mfcc_mean, mfcc_std, spectral_features,
        chroma_mean, chroma_std, rms_features
...
def predict_from_audio(audio_file):
    ...
    for win in windows:
        ...
        if hasattr(model, "predict_proba"):
            ...
            probs = _scores_to_probs(raw_scores[0] if raw_scores.ndim > 1 else raw_scores)
        else:
...
h1,h2,h3,h4,h5,h6,p,li,b,strong{color:#f1f5f9!important}
...
if st.session_state.stage == "input":
    ...
    has_audio_ready = bool(st.session_state.get("uploaded_temp_path"))
    st.markdown("<div style='height:14px'></div>", unsafe_allow_html=True)
    ...
    if has_audio_ready:
        ...
        <div style="background:rgba(59,130,246,0.08);
                    ...
                    font-weight:700;
                    font-family:'IBM Plex Mono',monospace;">
            Audio ready — run the scan
        </div>
        """, unsafe_allow_html=True)

    if has_audio_ready and st.button("Run Diagnostic Scan →"):
        with st.spinner("Processing acoustic signal..."):
            try:
                ...
                audio_name = st.session_state.get("uploaded_filename", "uploaded_audio.wav")
    ...
    render_car_expert_chatbot(
        vehicle_year=v_year,
        vehicle_make=v_make,
        vehicle_model=v_model,
        vehicle_miles=v_miles,
        selected_reference_class=st.session_state.get("selected_reference_class"),
        current_stage=st.session_state.get("stage", "input")
    )

# ==============================================================================
# 13. STAGE 2A: LOW CONFIDENCE
# ==============================================================================
    ...
    st.markdown("""
    <div class="ds-notice">
      <b>Notice:</b> The signal is not strongly separated. Review the top candidate classes below and compare them with the reference audio to confirm the correct fault category.
    </div>
    """, unsafe_allow_html=True)
...
# ==============================================================================
    ...
    <div class="ds-success">
      ...
      <code style="color:#6ee7b7;background:rgba(16,185,129,0.12);padding:3px 10px;border-radius:6px;font-size:15px;font-family:'IBM Plex Mono',monospace;font-weight:700;">{pretty_label(selected_label)}</code>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="ds-card">
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:14px;">
        ...
        <div>
          <div style="font-size:10px;font-weight:700;color:#475569;font-family:'IBM Plex Mono',monospace;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:5px;">Final Signal</div>
          <div style="font-size:17px;font-weight:800;color:#93c5fd;font-family:'IBM Plex Mono',monospace;">{pretty_label(selected_label)}</div>
        </div>
        <div>
          <div style="font-size:10px;font-weight:700;color:#475569;font-family:'IBM Plex Mono',monospace;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:5px;">Reference Clips</div>
          <div style="font-size:14px;font-weight:700;color:#64748b;font-family:'IBM Plex Mono',monospace;">{", ".join(selected_clips) if selected_clips else "None selected"}</div>
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
YOUR TASK:
Rewrite the 3 base recommendations into 3 clear actionable inspection steps.
Each step must:
- mention the {vehicle['year']} {vehicle['make']} {vehicle['model']} by name in at least one bullet
- reference the mileage ({vehicle['miles']:,} mi) where relevant
- be tied to "{pretty_label(selected_label)}"
- start with a concrete action verb

FORMAT:
Return exactly 3 lines each starting with "- ". No headers, no probabilities, no "may" or "might".
"""
